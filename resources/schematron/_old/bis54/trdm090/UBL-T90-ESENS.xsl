...
<!--PATTERN -->
	...
	<!--RULE -->
<xsl:template match="/ubl:Tender" priority="1006" mode="M4">
...
<xsl:choose>
			...
			<xsl:otherwise>
				<svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="not(count(//*[not(node()[not(self::comment())])]) &gt; 0)">
					...
					<svrl:text>A tender MUST not contain empty elements.</svrl:text>
				</svrl:failed-assert>
			</xsl:otherwise>
		</xsl:choose>
...
<xsl:choose>
			...
			<xsl:otherwise>
				<svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="(cbc:UBLVersionID)">
					...
					<svrl:text>A tender MUST have a syntax identifier.</svrl:text>
				</svrl:failed-assert>
			</xsl:otherwise>
		</xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cbc:UBLVersionID" priority="1005" mode="M4">
